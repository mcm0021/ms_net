...
class PorousMediaDataset(Dataset):
    def __init__(self, root_dir: str, scales: int, transform=None):
        ...
        for dir_path in sorted(self.root_dir.iterdir()):
            ...
            lbm_path = dir_path / "LBM.mat"
            mat_path = dir_path / f"{dir_path.name}.mat"

            if lbm_path.exists() and mat_path.exists():
                self.sim_image_paths.append(str(lbm_path))
                self.image_paths.append(str(mat_path))

        # self.image_paths = self.image_paths[:5]
        # self.sim_image_paths = self.sim_image_paths[:5]
...
def download_data(root_dir: str, max_workers: int = 5):
    ...
    with open('DRP-372_metadata (1).json') as f:
    ...
    download_tasks = download_tasks[:10]
...
def train(model: MS_Net,
          ...
          optimizer, scheduler, num_epochs, loss_function, accumulation_steps):
...
loss_f = nn.MSELoss()
loss_logs, val_logs = train(net, train_loader, test_loader, optimizer, scheduler, epochs, learning_rate, loss_f, EFFECTIVE_BATCH_SIZE)
